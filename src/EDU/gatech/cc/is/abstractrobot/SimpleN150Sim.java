...
//	int[] deadRobotsIds = {11, 7, 25, 39, 2, 14, 37, 1, 35, 30, 20, 15};
	int[] deadRobotsIds = {11, 7, 25, 39, 2, 14, 37, 1, 35, 30, 20, 15, 16, 6, 13,26,29,9,38, 12, 36,28,5,24,21};
	...
	public void init(double xp, double yp, double tp, double ignore,
		Color f, Color b, int v, double steerAngle, int indexOnEdge, boolean isLastOnEdge,
					 ...
					 double xEdgeStartVertex, double yEdgeStartVertex, int i, long s)
		{
		...
		this.edgeStartVertex = new Vec2(xEdgeStartVertex, yEdgeStartVertex);
...
//		steer.sett(steer.t + sturn);
	...
	public boolean AreAllLivingRobotsNearDestinationPoint(){
    ...
    public SimpleN150Sim GetRobot(int id){
		for (int i=0; i<all_objects.length; i++){
			if (all_objects[i].getID() == id){
				return (SimpleN150Sim) all_objects[i];
			}
		}
...
//		in_reverse = !in_reverse;
	...
	public Vec2 GetLeftPoint(long time, int robotId){
		SimpleN150Sim robot = GetRobot(robotId);
...
//		System.out.println("radAngle: " + radAngle);
	...
	public Vec2 GetRightPoint(long time, int robotId){
		SimpleN150Sim robot = GetRobot(robotId);
...
//		System.out.println("radAngle: " + radAngle);
	...
	private Vec2 GetPointByDistanceAndRadians(double distance, double radians, Vec2 position){
...
//		System.out.println("x = " + x + ", y = " + y);
	...
	public Circle2 GetFOV(int robotId){
		...
		return new Circle2(robotPosition, MultiForageN150.VISION_RANGE);
	}
...
//		double x = d / 3;
	...
	public double GetRobotSteer(int robotId){
		SimpleN150Sim robot = GetRobot(robotId);
		return robot.getSteerHeading(0L);
	}
	...
	*/
	...
	public double getTurretHeadingOfRobot(int robotId){
		SimpleN150Sim robot = GetRobot(robotId);
		return robot.getTurretHeading(0L);
	}
